...
-spec equal(ABM, ABM) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(ABN, ABN) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, ABO} | {error, any()}) -> ABO.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, ABT}) -> ABT.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(ABW)) -> ABW.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
